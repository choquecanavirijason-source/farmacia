...
class PurchaseController
{
    ...
    {
        $purchases = Purchase::query()->when($request->filled('search'), fn ($query) => $query->search((string) $request->query('search')))->sort((string) $request->query('sort_by', 'purchase_date'), (string) $request->query('sort_dir', 'desc'))->paginate(max(1, $request->integer('per_page', 10)));

        return $this->collectionResponse(PurchaseResource::collection($purchases), 'Compras obtenidas con éxito.');
    }
    ...
    public function store(StorePurchaseRequest $request)
    {
        ...
        return $this->createdResponse(new PurchaseResource($compra), 'Compra registrada con éxito y lotes ingresados al inventario.');
    }

    public function show(int $id)
    {
        return $this->resourceResponse(new PurchaseResource(Purchase::findOrFail($id)), 'Compra obtenida con éxito.');
    }
    ...
    public function update(UpdatePurchaseRequest $request, int $id)
    {
        ...
        return $this->updatedResponse(new PurchaseResource($item->refresh()), 'Compra actualizada con éxito.');
    }
    ...
    public function details(int $id)
    {
        ...
        $details = PurchaseDetail::where('purchase_id', $id)->orderBy('id')->get();
