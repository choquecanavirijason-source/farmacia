...
export function InvoiceSheet({ open, onOpenChange, sale }: InvoiceSheetProps) {
  ...
  const invoice = currentSale?.invoice;
  const details = (currentSale?.details && currentSale.details.length > 0)
    ? currentSale.details
    : (currentSale as any).items || [];
  ...
  const nitCi =
    ...
    currentSale?.client?.nit ||
    currentSale?.client?.ci ||
    (currentSale as any)?.nit_cliente ||
    "0";
            ...
            <div className="flex flex-col gap-4 p-4 mt-2 border rounded-lg bg-card text-xs">
              ...
              <div className="flex justify-between">
                <span>
                  N° Factura:{" "}
                  <strong>
                    {invoice?.invoice_number || `FAC-${currentSale.id.toString().padStart(6, "0")}`}
                  </strong>
                </span>
                <span>
                  Fecha:{" "}
                    ...
                    {details.length === 0 ? (
                      ...
                      details.map((d: any, idx: number) => {
                        const medName =
                          ...
                          d.name ||
                          `Medicamento #${d.medicament_id || idx + 1}`;
                        const medConc =
                          d.medicament?.concentration || d.medicamento?.concentracion || "";
              ...
              <div className="flex justify-between items-center pt-2 font-bold text-sm border-t">
                ...
                <span className="font-mono text-base text-primary">
                  {formatCurrency(Number(currentSale.total))}
                </span>
              </div>
            </div>
          ) : null}
